...
gen_base_model_name = "models/hyperclovax-1.5b-instruct"
gen_tokenizer = AutoTokenizer.from_pretrained(gen_base_model_name, local_files_only=True)
gen_base_model = AutoModelForCausalLM.from_pretrained(gen_base_model_name, local_files_only=True)
gen_model = PeftModel.from_pretrained(gen_base_model, MODEL_PATH).to(device)

# 평가용 모델 로드
eval_base_model_name = "models/exaone-3.5-2.4b-instruct"
eval_tokenizer = AutoTokenizer.from_pretrained(eval_base_model_name, local_files_only=True)
eval_model = AutoModelForCausalLM.from_pretrained(eval_base_model_name, local_files_only=True)

# LLMChain 생성
gen_llm_chain = create_gen_llm_chain(model=gen_model, tokenizer=gen_tokenizer, device=0 if device == "cuda" else -1)
eval_llm_chain = create_eval_llm_chain(model=eval_model, tokenizer=eval_tokenizer, device=0 if device == "cuda" else -1)
...
graph.add_edge("evaluate", "check_completion")
